...
if "search_results" not in st.session_state:
    st.session_state.search_results = None  # 마지막 검색 결과
if "search_query_display" not in st.session_state:
    st.session_state.search_query_display = None  # 마지막 검색어 (표시용)
if "smart_search_used" not in st.session_state:
    st.session_state.smart_search_used = False
...
def render_table_card(table: TableSearchResult, index: int):
    ...
    with st.container():
        ...
        # Action buttons (outside expander to avoid rerender issue)
        ...
        with col3:
            st.caption("코드 보기: 아래 테이블 펼침")

        st.markdown("---")

        qa_key = f"qa_input_{table.table_id}_{index}"
        qa_btn_key = f"qa_btn_{table.table_id}_{index}"

        with st.container():
            qa_cols = st.columns([4, 1])
            with qa_cols[0]:
                qa_question = st.text_input(
                    "이 테이블에 질문하기",
                    placeholder="예: 가장 매출이 높은 항목은? 총합은?",
                    key=qa_key,
                )
            with qa_cols[1]:
                st.markdown("<br>", unsafe_allow_html=True)
                qa_ask = st.button("🤖 질문", key=qa_btn_key, use_container_width=True)

            if qa_ask and qa_question:
                with st.status("🤖 AI가 답변을 생성 중...", expanded=False) as status:
                    try:
                        from pdftablesearch.llm_client import ZaiLLMClient

                        qa_table_html = table.table_html or table.table_markdown or ""
                        qa_table_title = table.table_title or "(제목 없음)"

                        client = ZaiLLMClient()
                        from datetime import date
                        today_str = date.today().strftime("%Y년 %m월 %d일")
                        system_prompt = (
                            f"You are a financial data analyst. Today is {today_str}. "
                            "Given a question and an HTML table, "
                            "provide a clear, accurate answer in Korean. Use specific numbers. "
                            "Keep answers to 2-3 sentences."
                        )
                        user_prompt = (
                            f"Question: {qa_question}\n\n"
                            f"Table (HTML):\n{qa_table_html[:3000]}\n\n"
                            f"Table Title: {qa_table_title}\n\nAnswer:"
                        )
                        ...
                        st.error(f"QA 오류: {exc}")

            if qa_key in st.session_state.qa_answers:
                st.success(f"💡 **답변:** {st.session_state.qa_answers[qa_key]}")

        st.markdown("---")

        with st.expander("📋 테이블 내용 & 코드 보기", expanded=False):
            ...
            # Always show code block below table
            st.markdown("#### 💻 HTML 코드 (복사 가능)")
...
tab_home, tab_docs = st.tabs(["🏠 홈", "📄 문서보기"])
...
with tab_home:
    ...
    if st.session_state.pdfs:
        ...
        st.markdown(
            ...
            <div style="display:flex; gap:16px; margin-bottom:16px;">
                <div style="background:#e8f4fd; padding:8px 16px; border-radius:8px; font-size:14px;">
                    📄 문서 <strong>{total_pdfs}</strong>개
                </div>
                <div style="background:#e8f8e8; padding:8px 16px; border-radius:8px; font-size:14px;">
                    📊 테이블 <strong>{total_tables}</strong>개
                </div>
            </div>
            """,
    ...
    # 소개 섹션
    st.markdown("## 👋 PDF 테이블 검색 시스템에 오신 것을 환영합니다")
    st.markdown(
        """
        PDF 문서에서 **테이블을 자동으로 추출**하고, **자연어로 검색**할 수 있는 시스템입니다.

        **주요 기능:**
        - 📊 **테이블 자동 추출** — PDF에서 표를 자동으로 인식하고 추출합니다
        - 🔍 **자연어 검색** — "연체율", "사업성 평가 결과" 등 자연어로 테이블을 찾습니다
        - 🧠 **AI 스마트 검색** — AI가 검색어에 가장 적합한 테이블을 추천합니다
        - 🤖 **테이블 Q&A** — 추출된 테이블에 대해 질문하고 답변을 받을 수 있습니다
        """
    )
    ...
    uploaded_files = st.file_uploader(
        ...
        help="검색할 PDF 파일을 선택하세요 (여러 파일 동시 업로드 가능)",
    ...
    if uploaded_files:
        ...
        st.info(f"📎 선택된 파일: **{file_names}**")

        if st.button("📥 업로드 처리", type="primary", use_container_width=True):
            import time
            with st.status("📄 PDF 변환 및 테이블 추출 중...", expanded=True) as status:
                progress_bar = st.progress(0, text="준비 중...")

                start_time = time.time()
                progress_bar.progress(20, text="📄 PDF 변환 및 테이블 추출 중...")
                ...
                st.session_state.last_upload_time = conversion_time
                ...
                status.update(
                    label=f"✅ 처리 완료! {success_count}개 문서, {total_tables}개 테이블 ({conversion_time:.1f}초)",
                    state="complete",
                    expanded=False,
                )
    ...
    if st.session_state.pdfs:
        ...
        if st.session_state.last_upload_time:
            st.caption(f"⏱️ 마지막 처리 시간: {st.session_state.last_upload_time:.1f}초")
            ...
            if pdf_data.get("status") == "success":
                with st.container():
                    ...
                    with cols[0]:
                        ...
                        st.markdown(
                            f"📄 **{filename}** — 테이블 {table_count}개"
                        )
                    with cols[1]:
                        if st.button("🗑️ 삭제", key=f"home_del_{filename}"):
                            del st.session_state.pdfs[filename]
                            st.rerun()
            else:
                st.error(f"❌ {filename}: {pdf_data.get('error', 'Unknown error')}")
        ...
        st.markdown("")
        st.markdown("👉 **[문서보기]** 탭으로 이동하여 테이블을 검색해보세요!")
        ...
        st.markdown(
            ...
            ">
                ...
                <div style="font-size: 18px; margin-bottom: 8px;">PDF 파일을 업로드해주세요</div>
                <div style="font-size: 14px;">위의 업로드 영역에 파일을 드래그하거나 클릭하세요</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
with tab_docs:
    if not st.session_state.pdfs:
        ...
        with col4:
            use_smart_search = st.checkbox("🧠 Smart Search", value=False, help="AI가 가장 적합한 테이블을 선택합니다")
                ...
                with st.spinner(f"'{query}' 검색 중..."):
                    ...
                    try:
                        ...
                        if all_pdfs:
                            ...
                            if use_smart_search:
                                ...
                                if pdf_path:
                                    ...
                                    def on_smart_progress(phase: str, message: str, pct: int):
                                    ...
                                    ai_result = smart_search(
                                        ...
                                        progress_callback=on_smart_progress,
                                    )
                        ...
                        if results:
                            ...
                            st.session_state.smart_search_used = use_smart_search
        ...
        # Display saved search results (if page was rerun due to download button)
        elif not search_button and st.session_state.search_results:
            # 검색어 입력창에 이전 검색어 유지
            if st.session_state.search_query_display:
                st.query_params["search_query"] = st.session_state.search_query_display

            results = st.session_state.search_results
            query = st.session_state.search_query_display
            use_smart_search = st.session_state.smart_search_used

            if results:
                if use_smart_search:
                    # Smart Search 결과
                    ai_count = 1
                    additional_count = len(results) - ai_count

                    st.success(f"🎉 AI가 선택한 가장 적합한 테이블 + {additional_count}개 추천!")
                    st.markdown("---")

                    if results:
                        st.markdown("### 🏆 AI가 선택한 테이블")
                        render_table_card(results[0], 0)
                        st.info(f"💡 Smart Search: LLM이 '{query}' 검색어에 가장 적합한 테이블을 선택했습니다.")
                    ...
                    st.success(f"🎉 {len(results)}개 테이블 찾음!")
                    st.markdown("---")
