#!/usr/bin/env bash
# Start a local OpenLDAP server for PDF-INTELLIGENCE-PORTAL development.
set -euo pipefail
...
# Required on macOS or slapd can overflow its fd limit probe during startup.
ulimit -n 4096
"$SLAPD" -f "$RUN_DIR/slapd.conf" -h "$LDAP_URL"
sleep 1

"$LDAPADD" -x -H "$LDAP_URL" -D "$ROOT_DN" -w "$ROOT_PW" \
    -f "$SCRIPT_DIR/seed.ldif"
